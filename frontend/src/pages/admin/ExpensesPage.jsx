...
export default function ExpensesPage() {
  ...
  const onDelete = async (id) => {
    ...
    setLoading(true);
    try { await deleteExpense(id); await load(); } finally { setLoading(false); }
  };
  ...
  return (
    <Layout title="Financeiro" breadcrumbs={['Dashboard', 'Financeiro', 'Despesas']}>
      {/* Header */}
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'} flex items-center gap-2`}>
            <CreditCard className="w-6 h-6 text-[#ff6600]" /> Despesas
          </h2>
          <p className={`${isDarkMode ? 'text-[#f3f7f1]' : 'text-gray-600'}`}>Cadastre e gerencie despesas por subconta e competência</p>
        </div>
        <button
      ...
      <div className="card">
        <div className="p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Lista de Despesas</h3>
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              <CreditCard className="w-12 h-12 text-[#ff6600]/40 mx-auto mb-4" />
              <p className={`${isDarkMode ? 'text-[#f3f7f1]/60' : 'text-gray-500'} mb-2`}>Nenhuma despesa cadastrada</p>
              <p className={`${isDarkMode ? 'text-white' : 'text-gray-900'} font-medium mb-4`}>Comece criando sua primeira despesa</p>
              <button
                onClick={handleCreateNew}
